...
Data is collected through the Steam WebAPI, as well as replay parsing of the .dem files.  The replay file contains much more data than the WebAPI, at the cost of significantly more CPU time.
...
This data comes from replay files, and it's usually due to the replay file being unavailable, which can have a variety of causes:
...
* Valve allows you to opt-out by having "Expose Public Match Data" unchecked in your Dota 2 options (which the default option).  This causes you to appear as anonymous in future matches, although existing data is not deleted.
